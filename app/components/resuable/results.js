...
const Results = ({ freeTutoringRefClick }) => {
  return (
    <section className="w-full bg-[#25272c] sm:h-[112vh] py-[20vw] sm:py-0 relative overflow-hidden flex items-center justify-center">
      <Image
        width={10000}
        height={10000}
        src="/grid/noline.png"
      ...
      <div className="w-full h-full flex flex-col z-10 items-center justify-center">
        <header className="flex flex-col items-center justify-center">
          ...
          >
            Discover the Results We&apos;ve Delivered So Far. Our Achievements Speak for Themselves.
          </motion.p>
        </header>
        <div className="flex flex-col sm:flex-row mt-[4vw] gap-[1.25vw] justify-between">
          {[
            { value: "1490", label: "SAT Average", color: "#3958F2" },
            { value: "75+", label: "USACO Promotions", color: "#F3B71D" },
            { value: "25%", label: "Test Improvement", color: "#F7711F" },
            { value: "20+", label: "AIME Qualifiers", color: "#F46036" },
          ].map((item, index) => (
            <motion.article
              key={index}
